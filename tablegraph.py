...
# Different ways to read fieldnames: --firstline, --fieldnames, autogenerated names (default)
printf "time x y\\n1 2 3\\n3 4 5\\n"  | chart.py --fieldsep ' ' --firstline table
printf "1 2 3\\n3 4 5\\n"  | chart.py --fieldsep ' ' --fieldnames 'time,x,y' table
...
CSVDictRows = ty.List[ty.Dict[str, str]]
...
def csv_to_dictrows_first_line_as_fieldnames(inputfile: ty.TextIO, delimiter: str) -> CSVDictRows:
    dictreader = csv.DictReader(inputfile, delimiter=delimiter)
    return [row for row in dictreader]


def csv_to_dictrows_gen_fieldnames(inputfile: ty.TextIO, delimiter: str) -> CSVDictRows:
...
class CSVRows(ty.NamedTuple):
    fieldnames: ty.List[str]
    rows: ty.List[ty.List[str]]
...
def csv_to_rows_gen_fieldnames(inputfile: ty.TextIO, delimeter: str) -> CSVRows:
    csvreader = csv.reader(inputfile, delimiter=delimeter)
    first_line = next(csvreader)
    fieldnames = [f"field_{i}" for i in range(len(first_line))]
    rows = [first_line]
    rows.extend(row for row in csvreader)
    return CSVRows(fieldnames=fieldnames, rows=rows)
...
def build_table(
    *,
    ...
    logger.debug("csv rows: %r", rows)
    assert len(rows) > 0, "No empty data!!"
    keys = rows.fieldnames
    table = DataTable(
        data=rows.rows,
        columns=[DataTableColumn(title=k) for k in keys],
